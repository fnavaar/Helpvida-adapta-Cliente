# LOOP-04 — Saúde operacional

**Status:** RASCUNHO — validar na call de setup  
**Fase de origem:** Fase 4  
**Sistemas usados:** Métricas das Fases 1–3  
...
- **O que atingir:** manter o ganho operacional sem degradar qualidade.
- **Prazo do ciclo:** [VALIDAR NA CALL DE SETUP].
- **Valor estimado em R$:** não estimado; sem fonte aprovada.

## 2. Validação

- **Como saber que foi atingido:** métrica primária = tempo mediano origem→aprovação interna; retrabalho é guarda separada e não pode ultrapassar o limite [VALIDAR NA CALL].
- **Modo:** o assistente mede; humano valida no fim.
- **Fonte independente:** relatório de mesma coorte/janela — disponibilidade e consulta [VALIDAR]. **[INFERÊNCIA — independência a declarar na call: hoje a fonte é o próprio sistema monitorado; independência real exige log append-only, export auditado ou amostragem humana]**
...
- **Quem valida:** responsável nomeado pela Gestão, no encerramento de cada ciclo [VALIDAR NOME].
...
Use somente a fonte aprovada e dados mínimos. Meça tempo mediano origem→aprovação interna, acompanhado de retrabalho. Não altere conta, campo, regra, prioridade, prazo ou acesso. Quando a fonte estiver incompleta ou houver risco de dado sensível, pause e peça validação. Registre evidência sanitizada, comparação com baseline e aprendizado específico; o veredito final é humano.
...
- **Condição de pausa:** o loop pausa o veredito quando coorte, janela, unidade ou guarda de retrabalho forem incomparáveis; Gestão decide retomada.
...
- **Rollback/recuperação:** suspender conclusão de ganho, marcar relatório como inconclusivo e voltar à medição manual da mesma coorte.
- **SPEC da fase 4:** SPEC-4-004 — planejada; ainda não gerada.
