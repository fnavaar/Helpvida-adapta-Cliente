# LOOP-03 — Pendências e SLA

**Status:** RASCUNHO — validar na call de setup  
**Fase de origem:** Fase 4  
**Sistemas usados:** F2 central determinística e fila de exceções  
**Agente responsável:** Assistente principal

## 1. Meta

- **O que atingir:** reduzir tempo parado em pendências.
- **Prazo do ciclo:** [VALIDAR NA CALL DE SETUP].
- **Valor estimado em R$:** não estimado; sem fonte aprovada.

## 2. Validação

- **Como saber que foi atingido:** tempo mediano entre abertura e resolução, com p90 ou percentual dentro do SLA como guarda. Unidade e limite [VALIDAR NA CALL].
- **Modo:** o assistente mede; humano valida no fim.
- **Fonte independente:** fila com abertura, owner, prazo, estado e resolução — disponibilidade e consulta [VALIDAR]. **[INFERÊNCIA — independência a declarar na call: hoje a fonte é o próprio sistema monitorado; independência real exige log append-only, export auditado ou amostragem humana]**
...
Use somente a fonte aprovada e dados mínimos. Meça tempo mediano entre abertura e resolução da pendência. Não altere conta, campo, regra, prioridade, prazo ou acesso. Quando a fonte estiver incompleta ou houver risco de dado sensível, pause e peça validação. Registre evidência sanitizada, comparação com baseline e aprendizado específico; o veredito final é humano.
...
- **Condição de pausa:** o loop pausa notificações quando owner, SLA ou canal não estiverem aprovados; Auditoria/Faturamento assume a fila manual.
...
- **Rollback/recuperação:** desativar lembretes, manter fila manual ordenada e registrar itens não notificados para reconciliação.
- **SPEC da fase 4:** SPEC-4-003 — planejada; ainda não gerada.
